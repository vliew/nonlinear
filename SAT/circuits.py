...
def writeSTORE_NOTEQUAL(col_constraints,col,e,a,b):
    col_constraints[col].append("-%d -%d -%d 0 \n" %(e,a,b))
    col_constraints[col].append("-%d %d %d 0 \n" %(e,a,b))
    col_constraints[col].append("%d -%d %d 0 \n" %(e,a,b))
    col_constraints[col].append("%d %d -%d 0 \n" %(e,a,b))
...
def writeXOR3(col_constraints, col,d,a_0,a_1,a_2):
    ...
    col_constraints[col].append("-%d %d %d %d 0 \n" %(d,a_0,a_1,a_2))
    col_constraints[col].append("%d -%d %d %d 0 \n" %(d,a_0,a_1,a_2))
    col_constraints[col].append("%d %d -%d %d 0 \n" %(d,a_0,a_1,a_2))
    col_constraints[col].append("%d %d %d -%d 0 \n" %(d,a_0,a_1,a_2))
    col_constraints[col].append("%d -%d -%d -%d 0 \n" %(d,a_0,a_1,a_2))
...
def writeFULLADDER(col_constraints, col, c_in, d_in, t, c_out, d_out):
    writeXOR3(col_constraints,col,d_out,c_in,d_in,t)
    writeMAJ(col_constraints,col,c_out,c_in,d_in,t)

def writeHALFADDER(col_constraints, col, d_in, t, c_out, d_out):
...
# len(o) = numBits+1
def writeRIPPLECARRYADDER(col_constraints,x,y,c,o,numBits, strip=False):
    if strip:
        writeHALFADDER(col_constraints, 0, y[0], x[0], c[0], o[0])
        for col in range(1,numBits):
            writeFULLADDER(col_constraints, col, c[col-1], y[col], x[col], c[col], o[col])
        writeEQUAL(col_constraints, numBits,c[numBits-1],o[numBits])
    else:
